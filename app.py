...
st.markdown('<p class="big-font">⚡ Miniko AI 戰略指揮室 (V21.0 深度報告版)</p>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.info("💡 V21 特點：AI深度戰略報告、布林逆勢勝率、費波詳解、波浪標示。")
...
def generate_deep_strategy(check, wave_d, wave_60, fib, df):
    ...
    summary = []
    if "3 浪" in wave_d:
        summary.append("🚀 **趨勢判定：主升段噴出中！** 目前日線處於最強勢的第3浪，所有均線多頭排列。")
    elif "4 浪" in wave_d:
        summary.append("⚠️ **趨勢判定：多頭回檔修正。** 日線進入第4浪整理，需觀察月線(22MA)支撐是否有效。")
    elif "空頭" in wave_d:
        summary.append("🛑 **趨勢判定：空頭架構。** 股價跌破年線，趨勢偏空，反彈皆是逃命波。")
    
    # --- 操作劇本 ---
    action = []
    if check['is_sop'] and check['warrant_5m']:
        action.append("🎯 **積極進攻：** SOP訊號亮燈 + 權證大戶進場，建議利用60分K拉回時積極切入。")
    elif check['is_buy_streak']:
        action.append("🛡️ **順勢操作：** 主力連續買超護盤，籌碼安定，可沿 5日線或10日線 操作。")
    elif bias > 10:
        action.append("⏳ **耐心等待：** 正乖離過大(股價衝太快)，短線隨時會回測月線，建議等拉回再買。")
    else:
    ...
    if bias > 15:
        tech_note.append(f"⚠️ **乖離率警示：** 目前乖離率 {bias:.2f}% (正乖離過大)，像橡皮筋拉到極限，隨時會回檔修正。")
